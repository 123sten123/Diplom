    ...
    <div role="main" id="main" class="cf">
        ...
        <div class="page-content">
            ...
            <div class="entry-content cf">
                ...
                <form:form method="POST" commandName="marketStock">
                    <form:select path="Market1" id="sels">
                        <form:option value="-" label="--Select phone"/>
                        <form:options items="${phonesMap}" />
                    </form:select>

                    <form:select path="Sctock1" id="sels1">
                        <form:option value="-" label="--Select phone"/>
                        <form:options items="${phonesMap2}" />
                    </form:select>

                    <form:select path="Sctock1" id="sels2">
                        <form:option value="-" label="--Select phone"/>
                        <form:options items="${phonesMap3}" />
                    </form:select>

                    <br/>

                    <form:select path="Market2" id="sels3">
                        <form:option value="-" label="--Select phone"/>
                        <form:options items="${phonesMap}" />
                    </form:select>

                    <form:select path="Sctock2" id="sels4">
                        <form:option value="-" label="--Select phone"/>
                        <form:options items="${phonesMap2}" />
                    </form:select>

                    <form:select path="Sctock2" id="sels5">
                        <form:option value="-" label="--Select phone"/>
                        <form:options items="${phonesMap3}" />
                    </form:select>
                    ...
                    <br/>
                    <spring:button class="link-button" >Расчитать</spring:button>
                ...
                <script>
                    showMessage(${CloseOne.charPriceClose}, ${CloseOne.charDate});
                </script>
                <div id="placeholder2" style="width:600px;height:300px;"></div>
                <script>
                    showMessage2(${CloseTwo.charPriceClose}, ${CloseTwo.charDate});
                </script>

                <form:form> method="post" action="add" commandName="contact">
                   Корреляция равна: ${Cor.rs};<<label for=""></label>>
                    <br/>
